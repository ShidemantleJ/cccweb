...
export default function CompetitorViewer(props) {
  ...
  const matches = data.matches.filter(
    (match) =>
      match.team1.names.includes(competitorName) ||
      match.team2.names.includes(competitorName)
  );
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center" }}>
        {getWcaId(competitorName) && (
          <a
            href={`https://worldcubeassociation.org/persons/${getWcaId(
              competitorName
            )}`}
            style={{
      ...
      {getTeam(competitorName) && (
        <h2
          onClick={() =>
            (window.location.href = `/teamstatistics/${getTeam(
              competitorName
            )}`)
          }
          style={{ cursor: "pointer" }}
        >
          Team: {getTeam(competitorName)}
        </h2>
      )}
      <h2>Solves:</h2>
      {matches.map((match) => (
