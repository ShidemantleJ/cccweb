...
function StreamStats () {
  ...
  switch (element) {
    ...
    case "team1times":
      return (
        ...
        {[...team1.times[team1.currentIndex]].reverse().map((time, index) => (
          <h1 style={{backgroundColor: '#bdbdbd', fontWeight: '600', marginBottom: '0px', marginTop: '0px'}} key={index}>{time}</h1>
        ))}
        </div>
      )
    case "team2times":
    ...
    case "team1recenttime":
      return <h1 style={{position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', marginBottom: '25px'}}>{team1.times[team1.currentIndex][team1.times[team1.currentIndex].length - 1]}</h1>
    case "team2recenttime":
      return <h1 style={{position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', marginBottom: '25px'}}>{team2.times[team2.currentIndex][team2.times[team2.currentIndex].length - 1]}</h1>
    default:
