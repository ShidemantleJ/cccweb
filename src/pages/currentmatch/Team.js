...
export default function Team() {
  ...
  const [misscramble, setMisscramble] = useState(false);
  ...
  useEffect(() => {
    const refs = {
      ...
      currentName: ref(
        database,
        `currentMatch/team${teamNum}/names/${currentIndex}`
      ),
      currentOpponentName: ref(
        database,
        `currentMatch/team${oppTeamNum}/names/${currentIndex}`
      ),
      scramble: ref(
        database,
        `currentMatch/scrambles/${currentIndex}/${scrambleIndex}`
      ),
    };
    ...
    const unsubscribes = [
      ...
      onValue(refs.teamStatus, (snapshot) => {
        ...
        if (data) {
          ...
          setMisscramble(data.misscramble || false);
        }
      }),
      ...
      <div style={{ width: "50%", margin: "0 auto", textAlign: "center" }}>
        ...
        >
          {competitorMayStart
            ...
            : scrambleChecked
            ? "Do not start, but your scramble has been checked"
            : misscramble
            ? "Misscramble. Please try the scramble again with another cube"
            : "Do not start"}
        </div>
      </div>
